...
\begin{itemize}
	\item[T0110] \textit{Installation:} Der Nutzer installiert mit \quotes{rion install \textit{Packagename}} ein Paket. Das Paket wird, mit eventuellen Abhängigkeiten, installiert. Skripte (Post-Install-Prozessierung) werden erfolgreich ausgeführt.
	...
	\item[T0140] \textit{Aktualisieren:} Der Nutzer akualiersiert mit \quotes{rion upgrade} alle installierten Pakete. Diese werden aktualisiert. Er aktualisiert mit \quotes{rion upgrade \textit{Packagename}} ein bestimmtes Paket, welches nun aktualisiert wird.
	...
	\item[T0131] RION kann sich mit \quotes{rion auth -u >>USER<< -p >>PWD<<} anmelden. RION kann sich auch über einen Configfile (nur für den Ersteller lesbar) anmelden. Sollte keine Anmeldung vorliegen fragt RION beim ersten Aufruf nach den Anmeldedaten.
...
\begin{itemize}
	\item[T0210] \textit{Paket hinzufügen:} Derr Nutzer fügt mit \quotes{inor add \textit{Packagename Packagefile}} ein Paket zur Datenbank hinzu, welches noch nicht in der Datenbank ist. Darauf befindet es sich in der Datanbank. Abhängigkeiten wurden berücksichtigt.

...
\begin{itemize}
	...
	\item[F0170] \textit{Installierte Pakete listen:} Der Nutzer erhält durch \quotes{\textit{rion list (package_name_pattern)}} eine Liste aller installierten Pakete, bzw aller installierten Funktionen eines Paketes.
	...
	\item[F0131] RION kann sich mit \quotes{rion auth -u >>USER<< -p >>PWD<<} anmelden. RION kann sich auch über einen Configfile (nur für den User lesbar) anmelden. Sollte keine Anmeldung vorliegen fragt RION beim ersten Aufruf nach den Anmeldedaten.
\end{itemize}

\section{INOR}

\begin{itemize}
	\item[F0210] \textit{Paket hinzufügen:} Sofern noch keine Version des Paketes in der Datenbank existiert, kann der Nutzer mit \quotes{inor add \textit{Packagename Packagefile}} ein Paket zur Datenbank hinzufügen.
	\item[F0220] \textit{Neue Version hinzufügen:} Der Nutzer kann mit \quotes{inor update \textit{Packagename Packageversion Packagefile}} eine neue Version eines Paketes zur Datenbank hinzufügen.
	...
	\item[F0260] \textit{Paket signieren:} Der Nutzer kann mit \quotes{inor sign} alle installierten alle Pakete in der Datenbank signieren oder er kann mit \quotes{rion sign \textit{Packagename (Versionsnummer)}} ein bestimmtes Paket oder auch nur eine bestimmte Version eines Paketes signieren.
